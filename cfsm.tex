...
** Composition must require $\I(M)$ to be deterministic ??.
...
\begin{definition}[CFSM embedding]
Let $M=(Q,q_0,\mathbb{A},\delta)$ and $M'=(Q',q'_0,\mathbb{A}',\delta')$
be two CFSMs.
We say that $M$ is {\em embedded} in $M'$ (${M}{M'}$) whenever there exists an injective $f:\delta\to\delta'$ such that
\begin{itemize}[-]
\item
$q_1\lts{l_1} q_2\lts{l_2} q_3$ in $M$ implies  $q'_2\to^*q''_2$ in $M'$ \\
where 
$f(q_1\lts{l_1} q_2)= q'_1\lts{l'_1} q'_2$ and $f(q_2\lts{l_2} q_3)= q''_2\lts{l'_2} q'_3$
\end{itemize}
...
Let $M_\ttr = (Q, q_0, \textit{Act}, \delta)$ and $M_\tts = (Q', q'_0, \textit{Act}, \delta')$  be two CFSMs.
We say that
\begin{enumerate}[i)]
\item
$M_\tts$ is {\em $\bm{\delta}$-complementary} with $M_\ttr$, written $\emb{\bm{\delta}}{M_\tts}{M_\ttr}$, whenever 
\begin{itemize}
\item[-] 
there exixts $M''_\ttr = (Q'', q''_0, \textit{Act}, \bm{\delta})\in\IDS(M_\ttr)$;
\item[-]
$M_\tts$ is structurally identical to $\I(M''_\ttr)$;
\item[-]
$q_1 \lts{\ttr\ttp!\msg[m]} q_2 \in \delta$ for some $\ttp$ \quad iff \quad 
$\eps(q_1)\lts{\ttp'\tts?\msg[m]}\eps(q_2)\in \delta'$ for some $\ttp'$;
\item[-]
$q_1 \lts{\ttp\ttr?\msg[m]} q_2 \in \delta$ for some $\ttp$ \quad iff \quad 
$\eps(q_1)\lts{\tts\ttp'!\msg[m]}\eps(q_2)\in \delta'$  for some $\ttp'$.
\end{itemize}
\end{enumerate}
\end{definition}

\begin{definition}[Partial Fusion]
Let $M_\hh = (Q, q_0, \textit{Act}, \delta)$ and $M_{\hh'} = (Q', q'_0, \textit{Act}, \delta')$
be two CFSMs such that $\emb{\bm{\delta}}{M_{\hh'}}{M_\hh}$.
We define the {\em partial fusion of $M_{\hh}$ and $M_{\hh'}$ via $\bm{\delta}$} as
$$\fusion_{\!\bm{\delta}}(M_{\hh},M_{\hh'}) = (Q\cup\widehat{Q},q_0,\textit{Act},\widehat{\delta})$$
...
where &  $\bullet$  & $\widehat{Q} =\Set{q^{(q, l,q')} \mid (q, l,q',\intf)\in\bm{\delta}}$; \\[1mm]
          &  $\bullet$  & $\widehat\delta = \Set{(q, l,q') \mid (q, l,q',\nintf)\in\bm{\delta}}\,\cup$\\ 
           &    & ${\hspace{20pt}}\Set{(q,\ttr\HH'?\msg[a],\widehat q), (\widehat q,\HH\tts!\msg[a],q') \mid  (q,\HH\tts!\msg[a],q',\intf)\in\bm\delta, (\eps(q),\ttr\KK?\msg[a],\ \eps(q'))\in\delta',\ \widehat q=q^{(q,\HH\tts!\msg[a],q')}}\, \cup$ \\
                &    & ${\hspace{20pt}}\Set{(q,\tts\HH?\msg[a],\widehat q), (\widehat q,\HH'\ttr!\msg[a],q') \mid  (q,\tts\HH?\msg[a],q',\intf)\in\bm\delta,\ (\eps(q),\hh'\ttr!\msg[a],\eps(q'))\in\delta',\ \widehat q=q^{(q,\tts\HH?\msg[a],q')}}.$
 \end{tabular} 
\end{definition}
