...
%
\item
If $M^2_\hh$ has no mixed states, then also $M^1_\hh$ has no mixed states. [[*Check*]]
%
\item
\label{fact:uniquesending-iv}
Let $q_{\HH}\not\in\widehat{Q_\hh}$ be a non final state of $M_\hh$. Then one and only one of the possible cases is possible.
\begin{enumerate}[a)]  
\item
There is a single outgoing transition from $q_\hh$ in $\delta_\hh$ and it is of the form
$(q_{\HH},\tts{\HH}?\msg[a],q'_\hh)$ or $(q_{\HH},{\HH}\tts?\msg[a],q'_\hh)$ with $q'_\hh\not\in\widehat{Q_\hh}$ and $\tts\in\roles_2$.
\item
There are $n>0$ outgoing transitions from $q_\hh$ in $\delta_\hh$, all of the form
$(q_{\HH},\ttu_j\HH?\msg[a]_j,\hat q_{\hh j})$ with $\hat q_{\hh j} \in\widehat{Q_\hh}$
with $1\leq j\leq n$. Moreover, if $M^2_\hh$ has no mixed states, either
...
By \cref{fact:uniquesending}(\ref{fact:uniquesending-i}) ${q_\hh}\not\in\widehat{Q_\hh}$.
